...
# Function to Send an Array to the Arduino
def send_data(array):
    data_str = ' '.join(map(str,array))
    serialCom.write((data_str + '\n').encode())
...
# Function to Convert from Indices of 2D Array to Algebraic Square Name
    ...
    try:
        for event in client.board.stream_game_state(game_id):
            if 'moves' in event:
                ...
                current_state[from_index] = 0
                
                # Sends the last move as an array to the Arduino so that it can update it's internal chess states 
                last_move_array = [from_index[0],from_index[1],to_index[0],to_index[1]]                
                send_data((last_move_array))
                time.sleep(3)
                break
